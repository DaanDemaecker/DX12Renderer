...
void DDM::RootSignature::Destroy()
{
...
void DDM::RootSignature::SetRootSignatureDesc(
...
uint32_t DDM::RootSignature::GetDescriptorTableBitMask(D3D12_DESCRIPTOR_HEAP_TYPE descriptorHeapType) const
{
...
uint32_t DDM::RootSignature::GetNumDescriptors(uint32_t rootIndex) const
{
    assert(rootIndex < 32);
